...
#make bcp as factor - could not order, maybe b/c it is a 1/0 and not a 1/2
alffCbf_subjDemos$bcp <- ifelse()
alffCbf_subjDemos$obcp <- as.factor(alffCbf_subjDemos$bcp)
...
for (i in 1:10242) {
  ...
  lh_lm_age_p_uncor[i] <- summary(age_lm_model)$coeff[4,4]  lh_lm_accuracy_p_uncor[i] <- summary(accuracy_lm_model)$coeff[5,4]
