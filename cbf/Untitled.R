...
accuracy_and_efficiency <- subset(cog, select = c("bblid", 
                                                  ...
                                                  "F3_Memory_Accuracy"))
#merge
subjDemos <- merge(subjDemos, accuracy_and_efficiency, by = "bblid")
...
#write output
write.table(subjDemos_with_lh_matrix, file = paste0(homedir, "/baller/results/cbf_accuracy/subjDemos_with_lh_", numrows, "x10242.csv"), sep = ",")
...
#write output
write.table(subjDemos_with_rh_matrix, file = paste0(homedir, "/baller/results/cbf_accuracy/subjDemos_with_rh_", numrows, "x10242.csv"), sep = ",")
...
for (hemi in hemis) {
  for (model in models) {
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi_model_p_unc, ".csv")
    ...
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi_model_t_unc, ".csv")
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi, "_gam_", model, "_p_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_p_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
    
    ### t
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi, "_gam_", model, "_t_fdr05.csv")
...
for (hemi in hemis) {
  for (model in lm_models) {
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi, "_lm_", model, "_p_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_p_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
    
    ### t
    filename <- paste0(homedir, "/baller/results/cbf_accuracy/", hemi, "_lm_", model, "_t_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_t_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
  }
}