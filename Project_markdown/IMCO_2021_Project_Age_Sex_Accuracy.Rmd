...
```
<img width="40%" src="/project/imco/baller/results/images/Mean_coupling_by_age_rplot_fdr.jpg"/>
...
for (analysis in analyses) {
  for (model in models) {
    for (corr in corrs) {  
      ...
      #Positive and negative vectors, contains Ts
      lh_outdir_pos_t <- paste0(homedir, "/baller/results/", stat_path, "lh_pos_", result_path, ".csv")
      rh_outdir_pos_t <- paste0(homedir, "/baller/results/", stat_path, "rh_pos_", result_path, ".csv")
      
      lh_outdir_neg_t <- paste0(homedir, "/baller/results/", stat_path, "lh_neg_", result_path, ".csv")
      rh_outdir_neg_t <- paste0(homedir, "/baller/results/", stat_path, "rh_neg_", result_path, ".csv")
     
      #pos and negative, all with masks
      ...
      #positive - assign everything < 0 to 0
      lh_stat_pos <- lh_stat_map
      lh_stat_pos[lh_stat_pos<0] <- 0
      
      rh_stat_pos <- rh_stat_map
      rh_stat_pos[rh_stat_pos<0] <- 0
      
      #negative - assign everything > 0 to 0
      lh_stat_neg <- lh_stat_map
      lh_stat_neg[lh_stat_neg>0] <- 0
      
      rh_stat_neg <- rh_stat_map
      rh_stat_neg[rh_stat_neg>0] <- 0
      ...
      #write output pos and neg maps
      write.table(x = lh_stat_pos, file = lh_outdir_pos_t, quote = F, row.names = F, col.names = F)
      write.table(x = rh_stat_pos, file = rh_outdir_pos_t, quote = F, row.names = F, col.names = F)
      
      write.table(x = lh_stat_neg, file = lh_outdir_neg_t, quote = F, row.names = F, col.names = F)
      write.table(x = rh_stat_neg, file = rh_outdir_neg_t, quote = F, row.names = F, col.names = F)
      
      #write output pos and neg maps, masked
...
** Preparing for GAM visualization **

One challenge in plotting GAMS is that every value for the linear term, whether positive or negative, will be plotted as a positive number in the GAM. This makes it difficult to interpret. As such, a way to adapt that is to take the sign of the T values from the linear term and multiple that times the F values, producing directionality for the GAM. Here, I take the gam term and linear term for age, extract the signage from the linear values, multiply it by the gam vector, and save.
```{r gam_T_and_F_value_vector}


### pre: gam and lm T maps
### post: gam t maps with signage from lm maps
### uses: Gives sinage to gam age maps based on the linear term in the model
### dependencies: and R will do. I used 3.2.5


#set remote homedirectories
#homedir = '/Users/eballer/BBL/imco/pmacs/PMACS_remote'
#homedir = '/project/imco'

lh_gam <- read.csv(paste0(homedir, "/baller/results/coupling_accuracy/lh_gam_age_t_fdr05.csv"), header = F)
rh_gam <- read.csv(paste0(homedir, "/baller/results/coupling_accuracy/rh_gam_age_t_fdr05.csv"), header = F)

lh_lm <- read.csv(paste0(homedir, "/baller/results/coupling_accuracy/lh_lm_age_t_fdr05.csv"), header = F)
rh_lm <- read.csv(paste0(homedir, "/baller/results/coupling_accuracy/rh_lm_age_t_fdr05.csv"), header = F)

lh_lm_sign <- ifelse(lh_lm$V1 > 0, 1, -1)
rh_lm_sign <- ifelse(rh_lm$V1 > 0, 1, -1)

lh_gam_with_lm_sign <- lh_gam$V1*lh_lm_sign
rh_gam_with_lm_sign <- rh_gam$V1*rh_lm_sign

write.table(lh_gam_with_lm_sign, paste0(homedir, "/baller/results/coupling_accuracy/lh_gam_with_lm_sign_age_t_fdr05.csv"), quote = F, row.names = F, col.names = F)
write.table(rh_gam_with_lm_sign, paste0(homedir, "/baller/results/coupling_accuracy/rh_gam_with_lm_sign_age_t_fdr05.csv"), quote = F, row.names = F, col.names = F)
```
...
** Spin test for Yeo parcellations in Sex differences contrast **
We would like to know whether the chances of brain regions within the FPN and DMN (Yeo 7) are by chance. To do that, first we will take the FDR corrected contrast, and trinarize it (1 if a value meets FDR correction, 0 if it does not, and -1 if that brain region is in the medial wall, or within region 8). Next, we will use two scripts Adam has recommended, in matlab, to 1) spin the vertices 1000x (scramble them up), and 2) Figure out whether the proportion of vertices within a specific network is by chance. Of note, a challenge of this vertex wise spin is the dang medial wall. As such, though my own personal FDR corrected things will never include medial wall, when I do the spin, if I don't account for this, I will guarantee that I'll never have more vertices in a network by chance, because they will always include medial wall which will always be negative.

To get around this, I will look at proportions. First, I'll take my 10242 maps of 1s, 0s, and -1s, and spin them round and round. In step 2, I'll first need 8 vectors that contain the vertices within each network, i.e. DMN: [102, 344, 5032]; FPN: [345 3492 5355]; and so on. I'll subset the spun matrix to look specifically at DMN. Next, I'll remove the values that have -1 (subset based on actual value, not location). Then, I'll calculate the # of vertices that are significant divided by all the vertices within that parcel. For example, if there are 100 parcels in the FPN overall, and in my spin, 20 are -1 (or medial), those will be removed. Then, I'll look for the proportion of 1s divided by the total # parcels left (80). So, to really be clear. I spin, I look at the FPN specifically, I remove the vertices that spun to have the medial wall, and then I say: how many are significant divided by how many there are total (minus medial wall). Those proportions will be saved. I'll then go back to R and make a nice little violin plot that will show the permuted proportions at each of the 7 parcels, and add a line for how many were in mine.

```{r yeo_spin_setup}
#pre: my FDR corrected sex differences coupling map, as well as the Yeo mapping
#post: Two vectors (lh, rh) of 1s (FDR corrected), 0 (not fdr corrected), -1 (in the medial wall), as well as vectors of the included vertices for each network
#uses: prepares for spin test, creates the vectors to be spun, as well as the files to be used when subsetting in part 2 of the spin
#dependencies- any R will do, I used 3.2.5

homedir = '/Users/eballer/BBL/imco/pmacs/PMACS_remote'
#homedir = '/project/imco'

#read in lh and rh FDR corrected plots
#### set # parcels in case I want to do 7 or 17 or something else in the future
parcel_type = "Yeo" 
parcel_num = 7 
input_parcel_array_length = 10242
...
for (analysis in analyses) {
  for (model in models) {
    for (corr in corrs) {  
      ...
      ## output
      lh_outdir <- paste0(homedir, "/baller/results/", stat_path, "lh_", result_path, "_", parcel_type, parcel_num, "_1_0_-1_spin.csv")
      rh_outdir <- paste0(homedir, "/baller/results/", stat_path, "rh_", result_path, "_", parcel_type, parcel_num, "_1_0_-1_spin.csv")
      ...
      #vectors for left
      lh_visual_values <- which(lh_numerical_map==1)
      lh_motor_values <- which(lh_numerical_map==2)
      lh_da_values <- which(lh_numerical_map==3)
      lh_va_values <- which(lh_numerical_map==4)
      lh_limbic_values <- which(lh_numerical_map==5)
      lh_fp_values <- which(lh_numerical_map==6)
      lh_dm_values <- which(lh_numerical_map==7)
      lh_mw_values <- which(lh_numerical_map==8)
      
      #vectors for right
      rh_visual_values <- which(rh_numerical_map==1)
      rh_motor_values <- which(rh_numerical_map==2)
      rh_da_values <- which(rh_numerical_map==3)
      rh_va_values <- which(rh_numerical_map==4)
      rh_limbic_values <- which(rh_numerical_map==5)
      rh_fp_values <- which(rh_numerical_map==6)
      rh_dm_values <- which(rh_numerical_map==7)
      rh_mw_values <- which(rh_numerical_map==8)
      
      ### convert my nice boolean matrix to have -1 wherever the medial wall would have been
      lh_stat_boolean[lh_mw_values,] <- -1
      rh_stat_boolean[rh_mw_values,] <- -1
      
      #and set all NAs to 0
      lh_stat_boolean[which(is.na(lh_stat_boolean))] <- 0
      rh_stat_boolean[which(is.na(rh_stat_boolean))] <- 0
      ...
      #write all the tables for parcel values
      
      #lh
      write.table(x = lh_visual_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_visual_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_motor_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_motor_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_da_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_da_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_va_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_va_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_limbic_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_limbic_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_fp_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_fp_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_dm_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_dm_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = lh_mw_values, file = paste0(homedir, "/baller/results/", stat_path, "lh_mw_values.csv"), quote = F, row.names = F, col.names = F)
     
      #rh
       write.table(x = rh_visual_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_visual_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_motor_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_motor_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_da_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_da_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_va_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_va_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_limbic_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_limbic_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_fp_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_fp_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_dm_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_dm_values.csv"), quote = F, row.names = F, col.names = F)
      write.table(x = rh_mw_values, file = paste0(homedir, "/baller/results/", stat_path, "rh_mw_values.csv"), quote = F, row.names = F, col.names = F)
