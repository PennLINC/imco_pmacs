...
20210309
 ...
 - moved Adam's files over to pmacs, including
    - SpinPermuFS.m, calc_spinDistribs_MAD_PG.m (renamed calc_spinDistribs_EB.m), SpinTest_MAD.m (remaned calc_spinTest_EB.m) 
    
    - SpinPermuFS.m
    
      - changed paths to freesurfer spheres: 
    ...
    - does permute distribution need to be in matlab or can I do in R?

20210310
  [x] doing the spin permutation calculation in R(spin_proportion_calculations_and_plots.R)
  [x] need to make yeo7 gam sex maps
  [x] repeat this analysis and do with exec accuracy
  [x] plot
  

20210311

  Notes from Ted Meeting

  [o] Table 1 - will be in body of text
  [] Figure 1 - Make text bigger. Otherwise, good
  [x] Figure 2 - Switch scatter and figure
      - A: Scatter
          - take out panel title
          - take out outer box
          [x] plot in visreg to get the gam and confidence interval
          [x] Do over all vertices, not just significant ones
      - B: Figure
          [o] ask Max if he can help with this
          [x] want grey for values that are 0 (i.e. not corrected)
          [x] can use this color scheme for the rest
          - make the upper part the lowest Z
  [x] Figure 3 - Spins
      - combine right and left
      - Maps look great.
      - To get confidence interval of spin
          [x] Rank all of the permutations for each network
          [x] Network is significant if the value is higher than 95% (or 950 or 1900)
      - Separate out positive and negative for spins
          - make pos and negative vectors with {r trinarize, eval=FALSE} in IMCO_2021_Project_Age_Sex_Accuracy.Rmd
          - [x] spin permutations - calc_spinTest_EB.m
          - [x] proportions
          - [x] plot 
      - On the violin plot (as it is now, with all significant stuff)
        - have a line for the positive direction
        - have a line for the negative direction
      [x] Make colors Yeo Colors
  [x] Figure 4 - Same as above
...
[] images: 
  ...
  [x] coupling schematic
    [x] caption
    [x] try to make prettier, using the maps we have now
      - red cbf map
      - command: PBP_vertWiseEffect_Erica_Ts_pos_and_neg_results_outpath('//project/imco/baller/results/mean_maps/lh_cbf_mean.csv','/project/imco/baller/results/mean_maps/rh_cbf_mean.csv','cbf_mean_fdr05')
      - blue alff map
...
ling_mean_fdr05_grpur')
  ...
  [x] coupling age
    ...
    [x] For scatterplots, average all vertices that fdr correct and correlate with age - just did all of them. Will need to move this to the bottom and save.
    
    [x] for yeo networks, perhaps pull out avg regression in FP vs Motor or something to show dissociation? 
      - average across fp7 for people?
      
  [x] coupling sex
    [x] caption
    [x] use gam picture, perhaps 1st (medial) and 3rd row (rotated to front)
      [x] ? yeo mapping - will do DLPFC plot
      [x] work with adam to try to get better scaling for the color so there is some gradation in the red. Could I use the range 3-6 instead of -2 to 6 or whatever? Should be easy enough - didn't look good. Will just keep as is
    [x] for yeo networks, look at FP males vs females -
  ...
  [x] coupling accuracy
    [x] caption
    [x] lm picture, probably medial and lateral picture (red/blue).
...
#### 20210317
[x] git page 
[x] start on methods/results

#### 20210401
[x] realized my accuracy should be a gam and that the accuracy gams should not have osex in them. Have changed that and am rerunning the other analyses.

#### 20210401
- ran coupling accuracy
[x] trinarize maps
[x] spin test
[x] proportions
[x] violin plots
[x] make picture
[x] continue with results
