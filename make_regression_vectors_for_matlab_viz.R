...
# read in demos
alffCbf_subjDemos <- read.csv("/project/imco/baller/subjectLists/n831_rest_cbf_finalSample_imageOrder.csv")
...
name(alffCbf_subjDemos) <- gsub("scanid.x", "scanid", names(alffCbf_subjDemos))
...
for (i in 1:10242) {
 ...
 # lh_gam_age_sex_p_uncor[i] <- summary(age_sex_model)$s.table[2,4] #smooth term for interaction, this was changed
  lh_gam_age_sex_p_uncor[i] <- summary(age_sex_intx_model)$s.table[2,4] #smooth term for interaction, this was changed
  
  #lh_gam_mood_p_uncor[i] <- summary(age_sex_model)$p.table[5,4]
  #lh_gam_psychopathology_p_uncor[i] <- summary(age_sex_model)$p.table[6,4]
  #lh_gam_accuracy_p_uncor[i] <- summary(age_sex_model)$p.table[7,4] #accuracy term
 ...
 # lh_gam_age_sex_t_uncor[i] <- summary(age_sex_model)$s.table[2,3] #smooth term for interaction
  lh_gam_age_sex_t_uncor[i] <- summary(age_sex_intx_model)$s.table[2,3] #smooth term for interaction
  
 # lh_gam_mood_t_uncor[i] <- summary(age_sex_model)$p.table[5,3]
  #lh_gam_psychopathology_t_uncor[i] <- summary(age_sex_model)$p.table[6,3]
  #lh_gam_accuracy_t_uncor[i] <- summary(age_sex_model)$p.table[7,3] #accuracy term
...
for (i in 1:10242) {
  ...
  # rh_gam_age_sex_p_uncor[i] <- summary(age_sex_model)$s.table[2,4] #smooth term for interaction, this was changed
  rh_gam_age_sex_p_uncor[i] <- summary(age_sex_intx_model)$s.table[2,4] #smooth term for interaction, this was changed
  
  #rh_gam_mood_p_uncor[i] <- summary(age_sex_model)$p.table[5,4]
  #rh_gam_psychopathology_p_uncor[i] <- summary(age_sex_model)$p.table[6,4]
  #rh_gam_accuracy_p_uncor[i] <- summary(age_sex_model)$p.table[7,4] #accuracy term
  ...
  # rh_gam_age_sex_t_uncor[i] <- summary(age_sex_model)$s.table[2,3] #smooth term for interaction
  rh_gam_age_sex_t_uncor[i] <- summary(age_sex_intx_model)$s.table[2,3] #smooth term for interaction
  
  # rh_gam_mood_t_uncor[i] <- summary(age_sex_model)$p.table[5,3]
  #rh_gam_psychopathology_t_uncor[i] <- summary(age_sex_model)$p.table[6,3]
  #rh_gam_accuracy_t_uncor[i] <- summary(age_sex_model)$p.table[7,3] #accuracy term
...
for (hemi in hemis) {
  for (model in models) {
    ...
    ## corrected ##
     ...
     ### p
    filename <- paste0("/project/imco/baller/processed_data/regression_matrices_n831/", hemi, "_gam_", model, "p_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", pfdr_for_matlab, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
    
     ### t
    filename <- paste0("/project/imco/baller/processed_data/regression_matrices_n831/", hemi, "_gam_", model, "t_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", tfdr_for_matlab, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
