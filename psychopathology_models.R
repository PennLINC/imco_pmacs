...
write.table(alffCbf_subjDemos_with_lh_831x10242, file = paste0(homedir, "/baller/results/psychopathology/alffCbf_subjDemos_with_lh_831x10242.csv"), sep = ",")
...
write.table(alffCbf_subjDemos_with_rh_831x10242, file = paste0(homedir, "/baller/results/psychopathology/alffCbf_subjDemos_with_rh_831x10242.csv"), sep = ",")
...
for (i in 1:10242) {
  ...
  psychopathology_model <- gam(lh_cbf_asl[,curcol] ~ pcaslRelMeanRMSMotion + restRelMeanRMSMotion +
                          osex + s(ageAtScan1, k = 4, fx = T) + overall_psychopathology + mood + psychosis + externalizing + phobias, data=lh_cbf_asl)
  ...
  psychopathology_lm_model <- lm(lh_cbf_asl[,curcol] ~ pcaslRelMeanRMSMotion + restRelMeanRMSMotion + ageAtScan1 + overall_psychopathology + mood + psychosis + externalizing + phobias,
                                 data=lh_cbf_asl)
  ...
  lh_gam_overall_psychopathology_p_uncor[i] <- summary(psychopathology_model)$p.table[5,4] 
  lh_gam_mood_p_uncor[i] <- summary(psychopathology_model)$p.table[6,4]
  lh_gam_psychosis_p_uncor[i] <- summary(psychopathology_model)$p.table[7,4]
  lh_gam_externalizing_p_uncor[i] <- summary(psychopathology_model)$p.table[8,4] 
  lh_gam_phobias_p_uncor[i] <- summary(psychopathology_model)$p.table[9,4] 
  
  
  #lm to assess directionality
  lh_lm_age_p_uncor[i] <- summary(age_lm_model)$coeff[4,4]
  
  lh_lm_overall_psychopathology_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[5,4] 
  lh_lm_mood_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[6,4]
  lh_lm_psychosis_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[7,4]
  lh_lm_externalizing_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[8,4] 
  lh_lm_phobias_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[9,4] 
  
  #put tvalue in it's appropriate lm
  lh_gam_age_t_uncor[i] <- summary(age_sex_model)$s.table[1,3] #smooth term for ageAtScan1
  
  lh_gam_overall_psychopathology_t_uncor[i] <- summary(psychopathology_model)$p.table[5,3] 
  lh_gam_mood_t_uncor[i] <- summary(psychopathology_model)$p.table[6,3]
  lh_gam_psychosis_t_uncor[i] <- summary(psychopathology_model)$p.table[7,3]
  lh_gam_externalizing_t_uncor[i] <- summary(psychopathology_model)$p.table[8,3] 
  lh_gam_phobias_t_uncor[i] <- summary(psychopathology_model)$p.table[9,3] 
  
  
  #lm to assess directionality
  lh_lm_age_t_uncor[i] <- summary(age_lm_model)$coeff[4,3]
  
  lh_lm_overall_psychopathology_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[5,3] 
  lh_lm_mood_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[6,3]
  lh_lm_psychosis_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[7,3]
  lh_lm_externalizing_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[8,3] 
  lh_lm_phobias_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[9,3] 
...
for (i in 1:10242) {
  ...
  psychopathology_lm_model <- lm(rh_cbf_asl[,curcol] ~ pcaslRelMeanRMSMotion + restRelMeanRMSMotion + ageAtScan1 + overall_psychopathology + 
                                   mood + psychosis + externalizing + phobias,
                                 data=rh_cbf_asl)
  ...
  rh_gam_overall_psychopathology_p_uncor[i] <- summary(psychopathology_model)$p.table[5,4] 
  rh_gam_mood_p_uncor[i] <- summary(psychopathology_model)$p.table[6,4]
  rh_gam_psychosis_p_uncor[i] <- summary(psychopathology_model)$p.table[7,4]
  rh_gam_externalizing_p_uncor[i] <- summary(psychopathology_model)$p.table[8,4] 
  rh_gam_phobias_p_uncor[i] <- summary(psychopathology_model)$p.table[9,4] 
  
  
  #lm to assess directionality
  rh_lm_age_p_uncor[i] <- summary(age_lm_model)$coeff[4,4]
  
  rh_lm_overall_psychopathology_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[5,4] 
  rh_lm_mood_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[6,4]
  rh_lm_psychosis_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[7,4]
  rh_lm_externalizing_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[8,4] 
  rh_lm_phobias_p_uncor[i] <- summary(psychopathology_lm_model)$coeff[9,4] 
  
  #put tvalue in it's appropriate lm
  rh_gam_age_t_uncor[i] <- summary(age_sex_model)$s.table[1,3] #smooth term for ageAtScan1
  
  rh_gam_overall_psychopathology_t_uncor[i] <- summary(psychopathology_model)$p.table[5,3] 
  rh_gam_mood_t_uncor[i] <- summary(psychopathology_model)$p.table[6,3]
  rh_gam_psychosis_t_uncor[i] <- summary(psychopathology_model)$p.table[7,3]
  rh_gam_externalizing_t_uncor[i] <- summary(psychopathology_model)$p.table[8,3] 
  rh_gam_phobias_t_uncor[i] <- summary(psychopathology_model)$p.table[9,3] 
  
  
  #lm to assess directionality
  rh_lm_age_t_uncor[i] <- summary(age_lm_model)$coeff[4,3]
  
  rh_lm_overall_psychopathology_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[5,3] 
  rh_lm_mood_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[6,3]
  rh_lm_psychosis_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[7,3]
  rh_lm_externalizing_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[8,3] 
  rh_lm_phobias_t_uncor[i] <- summary(psychopathology_lm_model)$coeff[9,3] 
...
for (hemi in hemis) {
  for (model in models) {
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi_model_p_unc, ".csv")
    ...
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi_model_t_unc, ".csv")
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi, "_gam_", model, "_p_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_p_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
    
    ### t
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi, "_gam_", model, "_t_fdr05.csv")
...
for (hemi in hemis) {
  for (model in lm_models) {
    ...
    ### p
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi, "_lm_", model, "_p_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_p_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
    
    ### t
    filename <- paste0(homedir, "/baller/results/psychopathology/", hemi, "_lm_", model, "_t_fdr05.csv")
    write_table_command <- paste0("write.table(x = ", hemi_model_t_fdr, ", file = \"", filename,"\", row.names = FALSE, col.names = FALSE)")
    eval(parse(text=write_table_command))
  }
}